...
export async function GET(
  ...
  try {
    ...
    if (presentation.slides) {
      presentation.slides.sort((a: any, b: any) => a.order_index - b.order_index);
    }
    ...
    if (progressError) {
      console.warn('Error fetching presentation progress:', progressError.message);
    ...
    if (presentation.slides && progress) {
      presentation.slides = presentation.slides.map((slide: any) => {
        const slideProgress = progress.find((p: any) => p.slide_id === slide.id);
        return {
          ...slide,
    ...
    return NextResponse.json({ error: `Failed to fetch presentation details: ${errorMessage}` }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    if (presentationError) {
      console.error('Error fetching presentation for deletion:', presentationError.message);
      if (presentationError.code === 'PGRST116') {
        return NextResponse.json({ error: 'Presentation not found.' }, { status: 404 });
      }
      return NextResponse.json({ error: 'Failed to verify presentation ownership.' }, { status: 500 });
    }
    ...
    const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred.';
    return NextResponse.json({ error: `Failed to delete presentation: ${errorMessage}` }, { status: 500 });
  }
}
