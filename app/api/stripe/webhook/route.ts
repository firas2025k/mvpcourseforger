...
async function updateUserSubscription(
  ...
  console.log(
    `Webhook Helper: Updating for User ID: ${userId}, App Plan ID: ${appPlanId}, Active: ${isActive}`
  );
  ...
  if (planError || !planData) {
    console.error(
      `Webhook Helper: Plan not found for app_plan_id ${appPlanId}:`,
      planError?.message
    );
  ...
  console.log(
    `Webhook Helper: Found plan "${planData.name}" with course limit ${planData.course_limit}`
  );
  ...
  if (stripeCurrentPeriodEnd) {
    subscriptionDataToUpsert.stripe_current_period_end =
  ...
  console.log(
    ...
    JSON.stringify(subscriptionDataToUpsert, null, 2)
  );
  ...
  if (subError) {
    console.error(
      `Webhook Helper: Error upserting subscription for user ${userId}:`,
      subError.message,
      subError.details
    );
    throw subError;
  ...
  console.log(
    `Webhook: Successfully updated subscription for user ${userId} to plan ${appPlanId} (${planData.name}). Active: ${isActive}. Profile course limit update handled by trigger.`
  );
}
  ...
  try {
    ...
    const { error: transactionError } = await supabase
      ...
      .insert({
        ...
        type: "purchase",
        amount: creditAmount,
        description: `Purchased ${packageName} - ${creditAmount} credits for $${amountPaid}`,
...
export async function POST(request: NextRequest) {
  ...
  if (relevantEvents.has(event.type)) {
    try {
      ...
      switch (event.type) {
        case "checkout.session.completed":
          ...
          if (
            ...
            console.log(
              "Webhook: checkout.session.completed for non-subscription or missing data."
            );
            break;
          }
          ...
          if (!userId || !appPlanId) {
            console.error(
              "Webhook Error: Missing supabase_user_id or app_plan_id in checkout session metadata."
            );
            break;
          }
          console.log(
            `Webhook: Processing checkout.session.completed for user ${userId}, app plan ID ${appPlanId}`
          );
        ...
        case "invoice.payment_succeeded":
          ...
          const subDataInvoice = await stripe.subscriptions.retrieve(
            subscriptionId
          );
          ...
          if (!userId || !appPlanId) {
            console.error(
              "Webhook Error: Missing supabase_user_id or app_plan_id in subscription metadata for invoice.payment_succeeded."
            );
            break;
          }
          console.log(
            `Webhook: Processing invoice.payment_succeeded for user ${userId}, app plan ID ${appPlanId}`
          );
          await updateUserSubscription(
            userId,
            appPlanId,
        ...
        case "customer.subscription.updated":
          ...
          if (!userId) {
            ...
            if (!userId) {
              console.error(
                "Webhook Error: Missing supabase_user_id in customer metadata for customer.subscription.updated."
              );
              break;
            }
          }
          ...
          if (willCancelAtPeriodEnd || !isActive) {
            ...
            const { data: freePlanData, error: freePlanError } =
              await supabaseAdmin
                .from("plans")
                .select("id")
                .eq("name", "Free")
            ...
            if (freePlanError || !freePlanData) {
              console.error(
                "Webhook: Free plan not found for inactive customer.subscription.updated. Cannot revert user properly."
              );
              appPlanId = null;
            } else {
            ...
            const { data: newPlanData, error: newPlanError } =
              await supabaseAdmin
                .from("plans")
                .select("id")
            ...
            if (newPlanError || !newPlanData) {
              console.error(
                `Webhook: Could not find app plan for Stripe price ID ${newStripePriceId} during subscription update.`
              );
              ...
              console.warn(
                `Webhook: Falling back to free plan for user ${userId} due to missing Stripe price ID.`
              );
              if (!freePlanData) {
                await updateUserSubscription(
                  userId,
          ...
          console.log(
            `Webhook: Processing customer.subscription.updated for user ${userId}, app plan ID ${appPlanId}, status ${subscriptionStatus}, cancel_at_period_end: ${willCancelAtPeriodEnd}, isActive: ${isActive}`
          );
          await updateUserSubscription(
            userId,
            appPlanId,
        ...
        case "customer.subscription.deleted":
          ...
          if (deletedSubscription.metadata?.supabase_user_id) {
            ...
            const { data: existingSubRecord, error: subRecordError } =
              await supabaseAdmin
                .from("subscriptions")
                .select("user_id")
            ...
            if (existingSubRecord) {
              ...
              console.error(
                `Webhook: Could not find user_id for deleted subscription ${subscriptionIdForDeletion}. Error: ${subRecordError?.message}`
              );
              try {
                ...
                if ((customer as any).metadata?.supabase_user_id) {
                  userIdForDeletion = (customer as any).metadata
                    .supabase_user_id;
                }
              } catch (e: any) {
                console.error(
                  `Webhook: Failed to retrieve customer ${customerIdForDeletion} for user_id fallback: ${e.message}`
                );
              }
            }
          }
          ...
          if (!userIdForDeletion) {
            console.error(
              `Webhook Error: Critical - Could not determine supabase_user_id for customer.subscription.deleted event for customer ${customerIdForDeletion}. Cannot revert plan.`
            );
            break;
          }
          ...
          if (freePlanError || !freePlanData) {
            console.error(
              "Webhook: Free plan not found for customer.subscription.deleted. Cannot revert user properly. Marking subscription inactive and course_limit to 0."
            );
            await supabaseAdmin
              .from("subscriptions")
              .update({
          ...
          console.log(
            `Webhook: Processing customer.subscription.deleted for user ${userIdForDeletion}. Reverting to app plan ID ${appPlanIdForDeletion}`
          );
          await updateUserSubscription(
            userIdForDeletion,
        ...
        case "invoice.payment_failed":
          ...
          console.log(
            `Webhook: Invoice payment failed for customer ${failedInvoice.customer}, subscription ${failedInvoice.subscription}. User might need to update payment method.`
          );
          break;
        ...
        case "payment_intent.succeeded": {
          ...
          if (paymentIntent.metadata.type === "credit_purchase") {
            await handleCreditPurchase(supabaseAdmin, paymentIntent);
          }
          break;
        }
      ...
      return NextResponse.json(
        { error: "Webhook handler failed. See server logs." },
        { status: 500 }
      );
    }
  } else {
